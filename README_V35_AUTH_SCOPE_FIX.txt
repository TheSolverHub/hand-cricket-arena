...
V35 LOGIN/MULTIPLAYER AUTH SCOPE FIX
- Fixed global multiplayer functions calling private authData() from the account IIFE.
- Added hcaGlobalAuthData() using the same hcaAuthV2 localStorage record.
- Multiplayer no longer depends on window.authData and is not broken by console/debug assignments to window.authData.
- Existing authenticated token/gameId flow is retained.

DEPLOY NOTE
- Replace the current Hand_Cricket_Arena.html in the deployed repo with this V35 file and commit/push it.
- Render should redeploy the same service; no database setting is required for this client-side fix.
- After deploy, hard-refresh the browser (Ctrl+Shift+R) and do not set window.authData to an object in DevTools.
- The correct client auth record is stored under localStorage key hcaAuthV2 and is read by hcaGlobalAuthData() in the global multiplayer client.
