...
if(req.method==='POST'&&(u==='/api/register'||u==='/api/login')){let body='';req.on('data',c=>body+=c);req.on('end',()=>{try{const m=JSON.parse(body||'{}'),identifier=String(m.identifier||'').trim().toLowerCase(),password=String(m.password||'');if(password.length<6)return json(res,400,{error:'Enter a valid login identifier and password of at least 6 characters.'});if(u==='/api/register'){if(!validId(identifier))return json(res,400,{error:'Use a valid email/phone and password of at least 6 characters.'});if(accounts[identifier])return json(res,409,{error:'Player account already exists.'});const requestedGameId=safeText(m.gameId||'',20);if(!validGameId(requestedGameId))return json(res,400,{error:'Game ID must be 3-20 letters, numbers or underscore.'});if(gameIdTaken(requestedGameId))return json(res,409,{error:'That Game ID is already taken.'});const playerId='HCA-'+crypto.randomBytes(5).toString('hex').toUpperCase();accounts[identifier]={playerId,gameId:requestedGameId,identifier,name:safeText(m.name||'Player',20),passwordHash:hash(password),role:'USER',subscription:{plan:'FREE',status:'FREE',source:'NONE',premium:false,startedAt:null,expiresAt:null,paymentId:null,verificationStatus:'NOT_REQUIRED'},createdAt:Date.now(),stats:{matches:0,wins:0,losses:0,runs:0,wickets:0,highest:0,fifties:0,hundreds:0,fours:0,sixes:0}};const acc=accounts[identifier];if((ownerGameId()&&requestedGameId.toLowerCase()===ownerGameId().toLowerCase())||(ownerEmail()&&identifier===ownerEmail()))acc.role='OWNER';saveAccounts();const tk=token();sessions.set(tk,playerId);const sub=subscriptionState(acc);return json(res,200,{ok:true,token:tk,playerId,gameId:requestedGameId,name:acc.name,role:acc.role,isAdmin:adminAllowedAccount(acc),subscription:sub});}const acc=accountForLogin(identifier);if(!acc||acc.passwordHash!==hash(password))return json(res,401,{error:'Invalid login details. Check your email/phone or Game ID and password.'});if(!acc.role)acc.role='USER';const sub=subscriptionState(acc);const tk=token();sessions.set(tk,acc.playerId);return json(res,200,{ok:true,token:tk,playerId:acc.playerId,gameId:acc.gameId,name:acc.name,role:acc.role,isAdmin:adminAllowedAccount(acc),subscription:sub});}catch(e){json(res,400,{error:'Invalid request.'})}});return;}
...
if(u==='/admin'){
 ...
 if(u==='/health'){return json(res,200,{ok:true,rooms:rooms.size,players:Object.keys(accounts).length,visits:analytics.visits||0,uniqueVisitors:analytics.uniqueVisitors||0,onlineSockets:wss.clients.size,persistence:pgPool?'postgres':'file'})};
...
wss.on('connection',ws=>{ws.on('message',raw=>{let m;try{m=JSON.parse(raw)}catch{return send(ws,{type:'error',message:'Invalid message'})}const name=String(m.name||'Player').slice(0,20);
 ...
 if(m.type==='team'){if(r.started||me.role)return;const t=String(m.team||'').toUpperCase();if(t!=='A'&&t!=='B')return;if(teamPlayers(r,t).length>=15)return send(ws,{type:'error',message:'Each playing team can have maximum 15 players.'});me.team=t;me.ready=false;assignCaptains(r);sendRoom(r);broadcastRoomList();return}
 if(m.type==='teamAdmin'){if(r.started||authorityLevel(r,me)<4)return;const pid=String(m.playerId||''),t=String(m.team||'').toUpperCase();if(!['A','B'].includes(t))return;const target=r.players.find(p=>p.id===pid&&!p.role);if(!target||teamPlayers(r,t).length>=15)return;target.team=t;target.ready=false;assignCaptains(r);sendRoom(r);return}
 ...
 if(m.type==='kick'||m.type==='ban'){if(r.started||authorityLevel(r,me)<4)return;const pid=String(m.playerId||'');const target=r.players.find(p=>p.id===pid);if(!target||target.id===me.id)return;if(m.type==='ban')r.banList.add(target.name.toLowerCase());r.banList.add(String(target.playerId||'').toLowerCase());send(target.ws,{type:'kicked',message:m.type==='ban'?'You were banned from this room.':'You were kicked from this room.'});try{target.ws.close()}catch(_){};r.players=r.players.filter(p=>p.id!==target.id);assignCaptains(r);sendRoom(r);broadcastRoomList();return}
