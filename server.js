...
const requireAdmin=req=>{const key=String(req.headers['x-admin-key']||'');const expected=String(process.env.ADMIN_KEY||'');return !!expected&&key===expected};
...
const authorityLevel=(r,p)=>{if(!p)return 0;if(r.roomAdminId===p.id)return 4;if(isCaptain(r,p))return 3;if(isReferee(r,p))return 2;if(r.officialIds?.has(p.id))return 1;return 0};
const canManage=(r,p)=>authorityLevel(r,p)>=2;
...
function assignCaptains(r){ for(const t of ['A','B']){ if(!r.players.some(p=>p.id===r.captains?.[t]&&p.online!==false)){ const q=r.players.find(p=>p.team===t&&p.online!==false); if(q) r.captains[t]=q.id; } } }
...
const server=http.createServer((req,res)=>{let u=(req.url||'/').split('?')[0];
 ...
 if(req.method==='POST'&&(u==='/api/register'||u==='/api/login')){let body='';req.on('data',c=>body+=c);req.on('end',()=>{try{const m=JSON.parse(body||'{}'),identifier=String(m.identifier||'').trim().toLowerCase(),password=String(m.password||'');if(!validId(identifier)||password.length<6)return json(res,400,{error:'Use a valid email/phone and password of at least 6 characters.'});if(u==='/api/register'){if(accounts[identifier])return json(res,409,{error:'Player account already exists.'});const requestedGameId=safeText(m.gameId||'',20);if(!validGameId(requestedGameId))return json(res,400,{error:'Game ID must be 3-20 letters, numbers or underscore.'});if(gameIdTaken(requestedGameId))return json(res,409,{error:'That Game ID is already taken.'});const playerId='HCA-'+crypto.randomBytes(5).toString('hex').toUpperCase();accounts[identifier]={playerId,gameId:requestedGameId,identifier,name:safeText(m.name||'Player',20),passwordHash:hash(password),role:'USER',subscription:{plan:'FREE',status:'FREE',source:'NONE',premium:false,startedAt:null,expiresAt:null,paymentId:null,verificationStatus:'NOT_REQUIRED'},createdAt:Date.now(),stats:{matches:0,wins:0,losses:0,runs:0,wickets:0,highest:0,fifties:0,hundreds:0,fours:0,sixes:0}};const acc=accounts[identifier];if((ownerGameId()&&requestedGameId.toLowerCase()===ownerGameId().toLowerCase())||(ownerEmail()&&identifier===ownerEmail()))acc.role='OWNER';saveAccounts();const tk=token();sessions.set(tk,playerId);const sub=subscriptionState(acc);return json(res,200,{ok:true,token:tk,playerId,gameId:requestedGameId,name:acc.name,role:acc.role,subscription:sub});}const acc=accounts[identifier];if(!acc||acc.passwordHash!==hash(password))return json(res,401,{error:'Invalid login details.'});if(!acc.role)acc.role='USER';const sub=subscriptionState(acc);const tk=token();sessions.set(tk,acc.playerId);return json(res,200,{ok:true,token:tk,playerId:acc.playerId,gameId:acc.gameId,name:acc.name,role:acc.role,subscription:sub});}catch(e){json(res,400,{error:'Invalid request.'})}});return;}
if(u==='/api/check-game-id'){const gid=String(new URL(req.url,'http://localhost').searchParams.get('gameId')||'').trim();if(!validGameId(gid))return json(res,400,{ok:false,available:false,error:'Game ID must be 3-20 letters, numbers or underscore.'});return json(res,200,{ok:true,available:!gameIdTaken(gid),gameId:gid});}
 if(u==='/api/me'){const a=accountForPlayerId(auth(req.headers['x-session-token']));if(!a)return json(res,401,{ok:false,error:'Unauthorized'});const sub=subscriptionState(a);return json(res,200,{ok:true,playerId:a.playerId,gameId:a.gameId,name:a.name,role:a.role||'USER',subscription:sub,stats:a.stats||{}});}
 ...
 if(u==='/api/subscription-info'){const a=accountForPlayerId(auth(req.headers['x-session-token']));if(!a)return json(res,401,{ok:false,error:'Unauthorized'});return json(res,200,{ok:true,subscription:subscriptionState(a)});}
...
wss.on('connection',ws=>{ws.on('message',raw=>{let m;try{m=JSON.parse(raw)}catch{return send(ws,{type:'error',message:'Invalid message'})}const name=String(m.name||'Player').slice(0,20);
 ...
 if(m.type==='create'){const acc=auth(m.token);if(!acc)return send(ws,{type:'error',message:'Please login/register first.'});const r={code:code(),password:'',matchType:String(m.matchType||'limited'),overs:Math.max(1,Math.min(50,Number(m.overs)||5)),banList:new Set(),captains:{A:null,B:null},teamNames:{A:'Team A',B:'Team B'},refereeId:null,commentatorId:null,officialIds:new Set(),players:[],roomAdminId:null,started:false,phase:'lobby',innings:1,score:0,wickets:0,balls:0,target:0,battingTeam:null,bowlingTeam:null,batterPos:0,bowlerPos:0,choices:new Map(),lockedUntil:0,gameOver:false,commentary:'Waiting for players...',firstInnings:null,toss:null,tossWinner:null,decision:null,ballHistory:[],inningsHistory:[],playerStats:{},mom:null};const p={id:id(),playerId:acc, name:accounts[Object.keys(accounts).find(k=>accounts[k].playerId===acc)]?.name||name,ws,ready:false,online:true,team:'A',role:null};r.players.push(p);r.roomAdminId=p.id;r.captains.A=p.id;rooms.set(r.code,r);sendRoom(r);broadcastRoomList();return}
 let r=rooms.get(String(m.room||'').toUpperCase());
 if(m.type==='join'){const acc=auth(m.token);if(!acc)return send(ws,{type:'error',message:'Please login/register first.'});if(!r)return send(ws,{type:'error',message:'Room not found.'});const existing=r.players.find(p=>p.playerId===acc&&p.online===false);if(existing){existing.ws=ws;existing.online=true;sendRoom(r);broadcastState(r);broadcastRoomList();return}if(r.started)return send(ws,{type:'error',message:'Match already started.'});if(r.banList.has(name.toLowerCase()))return send(ws,{type:'error',message:'You are banned from this room.'});const p={id:id(),playerId:acc,name:accounts[Object.keys(accounts).find(k=>accounts[k].playerId===acc)]?.name||name,ws,ready:false,online:true,team:'B',role:null};r.players.push(p);assignCaptains(r);sendRoom(r);broadcastRoomList();return}
 ...
 if(m.type==='chat'){const text=safeText(m.text,240);if(!text)return;const scope=m.scope==='team'?'team':'public';const msg={type:'chat',name:me.name,team:me.team,text,scope,at:Date.now()};if(scope==='team')sendTeam(r,me.team,msg);else broadcast(r,msg);return}
 ...
 if(m.type==='captain'){if(r.started||authorityLevel(r,me)<4 && authorityLevel(r,me)<2)return;const pid=String(m.playerId||'');const target=r.players.find(p=>p.id===pid&&!p.role);if(!target)return;assignCaptains(r);r.captains[target.team]=target.id;sendRoom(r);return}
 ...
 if(m.type==='switchActive'){if(!r.started||!['playing','decision','toss'].includes(r.phase)||authorityLevel(r,me)<3)return;const pid=String(m.playerId||''),target=r.players.find(p=>p.id===pid&&p.team===m.team&&p.role==null&&p.online!==false);if(!target)return;if(m.kind==='batter'){const arr=teamPlayers(r,r.battingTeam);const ix=arr.findIndex(p=>p.id===target.id);if(ix>=0)r.batterPos=ix;}else if(m.kind==='bowler'){const arr=teamPlayers(r,r.bowlingTeam);const ix=arr.findIndex(p=>p.id===target.id);if(ix>=0)r.bowlerPos=ix;}r.commentary=`🔄 ${me.name} switched ${m.kind} to ${target.name}.`;broadcastState(r);return}
