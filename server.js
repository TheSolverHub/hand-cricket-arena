...
const saveAccounts=()=>{
  ...
  persistChain=persistChain.then(async()=>{
    ...
    }catch(e){try{await client.query('ROLLBACK')}catch{};console.error('PostgreSQL account save failed:',e.message)}
    finally{client.release()}
  }).catch(e=>console.error('PostgreSQL persistence queue failed:',e.message));
};
...
const isPremiumAccount=acc=>{syncMembershipToSubscription(acc);return !!subscriptionState(acc).premium||isMembershipActive(acc)};
...
// ═══════════════════════════════════════════════════════
// HAND CRICKET ARENA — COMPLETE PACKAGE V1.0
// Membership + Name Verify + Launch Offer + Weekend Championship
// Integrated safely into the existing V38 server.
// ═══════════════════════════════════════════════════════
const GAME_CONFIG = {
  MEMBERSHIP_BASE_COINS: 100000,
  MEMBERSHIP_BASE_DAYS: 20,
  MEMBERSHIP_MONTH_INR: 19,
  MEMBERSHIP_6MO_INR: 49,
  MEMBERSHIP_YEAR_INR: 99,
  NAME_VERIFY_INR: 50,
  LAUNCH_TOTAL_USERS: 200,
  LAUNCH_TIERS: [
    { from: 1, to: 50, discount: 100, label: 'First 50 — FREE for 1 year' },
    { from: 51, to: 100, discount: 70, label: 'Next 50 — 70% OFF (₹30/year)' },
    { from: 101, to: 150, discount: 40, label: 'Next 50 — 40% OFF (₹60/year)' },
    { from: 151, to: 200, discount: 15, label: 'Next 50 — 15% OFF (₹85/year)' }
  ],
  WEEKEND: {
    START_DAY: 6, START_HOUR_IST: 19, START_MINUTE_IST: 0,
    REGISTRATION_MAX: 140, MIN_PLAYERS: 28,
    TEAM_SIZE: 7, MAX_TEAMS: 20, MIN_TEAMS: 4,
    OVERS: 15, BALL_TIMER_SEC: 25, WICKETS: 10,
    MAX_BAT_USES: 3, MAX_OVERS_PER_BOWLER: 4,
    PRIZE_COINS: 5000
  }
};
const MEMBERSHIP_TIERS = [
  { coins: 50000, days: 10, bonusDays: 0 },
  { coins: 100000, days: 20, bonusDays: 0 },
  { coins: 250000, days: 50, bonusDays: 5 },
  { coins: 500000, days: 100, bonusDays: 15 },
  { coins: 1000000, days: 200, bonusDays: 40 }
];
const MEMBERSHIP_FILE = path.join(ROOT, 'memberships.json');
let memberships = {};
try { memberships = JSON.parse(fs.readFileSync(MEMBERSHIP_FILE, 'utf8') || '{}'); } catch {}
function saveMemberships(){try{fs.writeFileSync(MEMBERSHIP_FILE,JSON.stringify(memberships,null,2))}catch{}}
function isMembershipActive(acc){if(!acc)return false;const m=memberships[acc.playerId];return !!m&&m.status==='ACTIVE'&&(!m.expiresAt||Number(m.expiresAt)>=Date.now())}
function getMembership(acc){if(!acc)return {status:'NONE',daysLeft:0};const m=memberships[acc.playerId];if(!m)return {status:'NONE',daysLeft:0,active:false};const daysLeft=m.expiresAt?Math.max(0,Math.floor((m.expiresAt-Date.now())/86400000)):0;return {...m,daysLeft,active:isMembershipActive(acc)}}
function grantMembership(playerId,days,source,meta){const now=Date.now(),existing=memberships[playerId],baseTime=(existing&&existing.expiresAt>now)?existing.expiresAt:now,newExpiry=baseTime+(days*86400000);memberships[playerId]={playerId,status:'ACTIVE',source:source||'COINS',startedAt:existing?.startedAt||now,renewedAt:now,expiresAt:newExpiry,totalDays:(existing?.totalDays||0)+days,meta:meta||{}};saveMemberships();return memberships[playerId]}
const NAME_VERIFY_FILE=path.join(ROOT,'name_verifications.json');
let nameVerifications={};try{nameVerifications=JSON.parse(fs.readFileSync(NAME_VERIFY_FILE,'utf8')||'{}')}catch{}
function saveNameVerify(){try{fs.writeFileSync(NAME_VERIFY_FILE,JSON.stringify(nameVerifications,null,2))}catch{}}
function isNameVerified(acc){if(!acc)return false;const v=nameVerifications[acc.playerId];return !!v&&v.status==='VERIFIED'}
const LAUNCH_FILE=path.join(ROOT,'launch_users.json');
let launchUsers={count:0,users:{}};try{launchUsers=Object.assign(launchUsers,JSON.parse(fs.readFileSync(LAUNCH_FILE,'utf8')||'{}'))}catch{}
function saveLaunch(){try{fs.writeFileSync(LAUNCH_FILE,JSON.stringify(launchUsers,null,2))}catch{}}
function registerLaunchUser(playerId,gameId){if(launchUsers.users[playerId])return launchUsers.users[playerId];if(launchUsers.count>=GAME_CONFIG.LAUNCH_TOTAL_USERS)return null;launchUsers.count++;const tier=GAME_CONFIG.LAUNCH_TIERS.find(t=>launchUsers.count>=t.from&&launchUsers.count<=t.to);const offer={playerId,gameId,position:launchUsers.count,tier:tier?tier.label:'NONE',discount:tier?tier.discount:0,finalPrice:tier?Math.round(99*(100-tier.discount)/100):99,registeredAt:Date.now(),claimed:false};launchUsers.users[playerId]=offer;saveLaunch();return offer}
const WEEKEND_FILE=path.join(ROOT,'weekend_tournaments.json');
let weekendTournaments={};try{weekendTournaments=JSON.parse(fs.readFileSync(WEEKEND_FILE,'utf8')||'{}')}catch{}
function saveWeekend(){try{fs.writeFileSync(WEEKEND_FILE,JSON.stringify(weekendTournaments,null,2))}catch{}}
function getISTNow(){const now=new Date();const istOffset=5.5*60*60*1000;return new Date(now.getTime()+(now.getTimezoneOffset()*60000)+istOffset)}
function getNextSaturday7PM(){const ist=getISTNow(),target=new Date(ist),daysUntilSat=(6-target.getDay()+7)%7;target.setDate(target.getDate()+daysUntilSat);target.setHours(19,0,0,0);if(target.getTime()<=ist.getTime())target.setDate(target.getDate()+7);return target.getTime()}
function ensureWeekendTournament(){const ist=getISTNow();const weekKey=ist.getFullYear()+'-W'+Math.ceil(ist.getDate()/7);let existing=Object.values(weekendTournaments).find(t=>t.weekKey===weekKey);if(existing)return existing;const id='WT-'+ist.getFullYear()+'-'+(ist.getMonth()+1)+'-'+ist.getDate();const tournament={id,weekKey,name:'Weekend Championship — '+ist.toDateString(),createdAt:Date.now(),startTime:getNextSaturday7PM(),status:'registration',players:[],teams:[],matches:[],winner:null,config:GAME_CONFIG.WEEKEND};weekendTournaments[id]=tournament;saveWeekend();console.log('🏆 Weekend tournament created:',id);return tournament}
function autoCreateTeams(tournament){const players=[...tournament.players];if(players.length<GAME_CONFIG.WEEKEND.MIN_PLAYERS){tournament.status='cancelled';tournament.reason='Not enough players';saveWeekend();return}for(let i=players.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[players[i],players[j]]=[players[j],players[i]]}const numTeams=Math.min(Math.floor(players.length/7),GAME_CONFIG.WEEKEND.MAX_TEAMS);const teamNames=['Thunder XI','Royal Strikers','Fire Warriors','Golden Tigers','Blue Panthers','Red Dragons','Silver Sharks','Iron Lions','Storm Riders','Shadow Warriors','Green Cobras','Cosmic Kings','Phoenix Rising','Steel Titans','Nitro Blasters','Phantom XI','Wild Hunters','Crystal Warriors','Sky Riders','Emerald Lords'];tournament.teams=[];for(let i=0;i<numTeams;i++){const teamPlayers=players.slice(i*7,(i+1)*7);tournament.teams.push({id:'TEAM-'+(i+1),name:teamNames[i%teamNames.length],players:teamPlayers.map(p=>({playerId:p.playerId,gameId:p.gameId,name:p.name})),captainId:teamPlayers[0]?.playerId,designatedBatsman:teamPlayers[0]?.playerId,batUses:{},bowlerOvers:{},wins:0,losses:0,points:0})}tournament.status='live';tournament.startedAt=Date.now();saveWeekend();console.log('🏆 Teams created:',tournament.teams.length)}
// Membership bought with coins also grants access to Premium multiplayer rooms.
function syncMembershipToSubscription(acc){if(!acc)return;const active=isMembershipActive(acc);acc.subscription=acc.subscription||{plan:'FREE',status:'FREE',source:'NONE',premium:false,startedAt:null,expiresAt:null,paymentId:null,verificationStatus:'NOT_REQUIRED'};if(active){const m=memberships[acc.playerId];acc.subscription={...acc.subscription,plan:'MEMBERSHIP',status:'ACTIVE',source:'MEMBERSHIP',premium:true,startedAt:m.startedAt,expiresAt:m.expiresAt,paymentId:null,verificationStatus:'NOT_REQUIRED'}}else if(acc.subscription.source==='MEMBERSHIP'){acc.subscription={...acc.subscription,plan:'FREE',status:'FREE',source:'NONE',premium:false,expiresAt:null}}}


// Weekend scheduler — checks every minute and creates teams when Saturday 7 PM IST is reached.
setInterval(()=>{try{const t=ensureWeekendTournament();if(t.status==='registration'&&Date.now()>=t.startTime){console.log('🏆 Weekend start time reached!');autoCreateTeams(t);if(typeof wss!=='undefined'){wss.clients.forEach(ws=>send(ws,{type:'weekendStart',tournamentId:t.id}))}}}catch(e){}},60000);
  ...
  let body='';req.on('data',c=>body+=c);req.on('end',async()=>{
    try{
      ...
      if(acc){
        ...
        registerLaunchUser(acc.playerId,acc.gameId);syncMembershipToSubscription(acc);saveAccounts();
    ...
    }catch(e){return json(res,401,{ok:false,error:e.message||'Google authentication failed.'})}
  });return;
}
...
if(u==='/api/check-game-id'){const gid=String(new URL(req.url,'http://localhost').searchParams.get('gameId')||'').trim();if(!validGameId(gid))return json(res,400,{ok:false,available:false,error:'Game ID must be 3-20 letters, numbers or underscore.'});return json(res,200,{ok:true,available:!gameIdTaken(gid),gameId:gid});}
 if(u==='/api/me'){const a=auth(req.headers['x-session-token']);if(!a)return json(res,401,{ok:false,error:'Unauthorized'});const sub=subscriptionState(a);return json(res,200,{ok:true,playerId:a.playerId,gameId:a.gameId,name:a.name,role:a.role||'USER',subscription:sub,stats:a.stats||{}});}

 // ═══════ COMPLETE PACKAGE V1.0 APIs ═══════
 if(u==='/api/membership/status'&&req.method==='GET'){
   const acc=auth(req.headers['x-session-token']);if(!acc)return json(res,401,{ok:false,error:'Login required'});syncMembershipToSubscription(acc);return json(res,200,{ok:true,membership:getMembership(acc),tiers:MEMBERSHIP_TIERS,realMoneyOptions:[{label:'1 Month',priceINR:19,days:30},{label:'6 Months',priceINR:49,days:180},{label:'1 Year',priceINR:99,days:365}],payments:'Google Play Billing is required for real-money purchases.'});
 }
 if(req.method==='POST'&&u==='/api/membership/buy-coins'){
   let body='';req.on('data',c=>body+=c);req.on('end',()=>{try{const acc=auth(req.headers['x-session-token']);if(!acc)return json(res,401,{ok:false,error:'Login required'});const m=JSON.parse(body||'{}'),tier=MEMBERSHIP_TIERS.find(t=>t.coins===Number(m.tierCoins));if(!tier)return json(res,400,{ok:false,error:'Invalid tier'});const coins=Number(acc.stats?.coins||0);if(coins<tier.coins)return json(res,400,{ok:false,error:'Need '+tier.coins+' coins. You have '+coins});acc.stats=acc.stats||{};acc.stats.coins=coins-tier.coins;const days=tier.days+tier.bonusDays;grantMembership(acc.playerId,days,'COINS',{coins:tier.coins});syncMembershipToSubscription(acc);saveAccounts();return json(res,200,{ok:true,message:'Membership for '+days+' days!',membership:getMembership(acc),coinsLeft:acc.stats.coins})}catch(e){return json(res,400,{ok:false,error:e.message})}});return;
 }
 if(req.method==='POST'&&u==='/api/membership/buy-money'){
   // Never mark an unpaid real-money order as successful. Real purchases must be verified by Google Play Billing on Android.
   return json(res,501,{ok:false,error:'Real-money membership is not enabled on the web server. Use Google Play Billing verification in the Android app.'});
 }
 if(u==='/api/launch/status'&&req.method==='GET'){
   const acc=auth(req.headers['x-session-token']);const myOffer=acc?(launchUsers.users[acc.playerId]||registerLaunchUser(acc.playerId,acc.gameId)):null;return json(res,200,{ok:true,totalUsers:GAME_CONFIG.LAUNCH_TOTAL_USERS,registered:launchUsers.count,remaining:Math.max(0,GAME_CONFIG.LAUNCH_TOTAL_USERS-launchUsers.count),tiers:GAME_CONFIG.LAUNCH_TIERS,myOffer});
 }
 if(req.method==='POST'&&u==='/api/launch/claim'){
   const acc=auth(req.headers['x-session-token']);if(!acc)return json(res,401,{ok:false,error:'Login required'});const offer=launchUsers.users[acc.playerId];if(!offer)return json(res,404,{ok:false,error:'No launch offer'});if(offer.claimed)return json(res,400,{ok:false,error:'Already claimed'});offer.claimed=true;offer.claimedAt=Date.now();saveLaunch();return json(res,200,{ok:true,message:'Claimed! '+offer.tier,offer});
 }
 if(u==='/api/name-verify/status'&&req.method==='GET'){
   const acc=auth(req.headers['x-session-token']);if(!acc)return json(res,401,{ok:false,error:'Login required'});const v=nameVerifications[acc.playerId];return json(res,200,{ok:true,verified:isNameVerified(acc),currentName:acc.name,request:v||null,priceINR:GAME_CONFIG.NAME_VERIFY_INR});
 }
 if(req.method==='POST'&&u==='/api/name-verify/request'){
   let body='';req.on('data',c=>body+=c);req.on('end',()=>{try{const acc=auth(req.headers['x-session-token']);if(!acc)return json(res,401,{ok:false,error:'Login required'});const m=JSON.parse(body||'{}'),newName=String(m.name||'').trim();if(newName.length<2||newName.length>20)return json(res,400,{ok:false,error:'Name must be 2-20 chars'});const takenBy=Object.values(nameVerifications).find(v=>v.status==='VERIFIED'&&String(v.requestedName||'').toLowerCase()===newName.toLowerCase());if(takenBy&&takenBy.playerId!==acc.playerId)return json(res,409,{ok:false,error:'Name already verified by another player'});nameVerifications[acc.playerId]={playerId:acc.playerId,gameId:acc.gameId,oldName:acc.name,requestedName:newName,status:'PENDING_PAYMENT',requestedAt:Date.now(),priceINR:GAME_CONFIG.NAME_VERIFY_INR};saveNameVerify();return json(res,200,{ok:true,message:'Request created. Real payment verification is required.',request:nameVerifications[acc.playerId]})}catch(e){return json(res,400,{ok:false,error:e.message})}});return;
 }
 if(req.method==='POST'&&u==='/api/name-verify/confirm-payment'){
   return json(res,501,{ok:false,error:'Name verification payment is not enabled on the web server. Verify the ₹50 Google Play purchase before confirming the name.'});
 }
 if(u==='/api/weekend/current'&&req.method==='GET'){
   const t=ensureWeekendTournament();return json(res,200,{ok:true,tournament:{id:t.id,name:t.name,status:t.status,startTime:t.startTime,timeLeftMs:Math.max(0,t.startTime-Date.now()),players:t.players.length,maxPlayers:GAME_CONFIG.WEEKEND.REGISTRATION_MAX,minPlayers:GAME_CONFIG.WEEKEND.MIN_PLAYERS,teams:t.teams.length,maxTeams:GAME_CONFIG.WEEKEND.MAX_TEAMS,minTeams:GAME_CONFIG.WEEKEND.MIN_TEAMS,config:GAME_CONFIG.WEEKEND,teams_list:t.teams.map(tm=>({id:tm.id,name:tm.name,players:tm.players.map(p=>p.name),playerCount:tm.players.length,captainId:tm.captainId})),winner:t.winner,prizeCoins:GAME_CONFIG.WEEKEND.PRIZE_COINS}});
 }
 if(req.method==='POST'&&u==='/api/weekend/register'){
   const acc=auth(req.headers['x-session-token']);if(!acc)return json(res,401,{ok:false,error:'Login required'});const t=ensureWeekendTournament();if(t.status!=='registration')return json(res,400,{ok:false,error:'Registration closed'});if(t.players.length>=GAME_CONFIG.WEEKEND.REGISTRATION_MAX)return json(res,400,{ok:false,error:'Full'});if(t.players.find(p=>p.playerId===acc.playerId))return json(res,400,{ok:false,error:'Already registered'});t.players.push({playerId:acc.playerId,gameId:acc.gameId,name:acc.name,registeredAt:Date.now()});saveWeekend();return json(res,200,{ok:true,message:'Registered! '+t.players.length+' in. '+(GAME_CONFIG.WEEKEND.REGISTRATION_MAX-t.players.length)+' left.',totalPlayers:t.players.length});
 }
 if(u==='/api/weekend/my-team'&&req.method==='GET'){
   const acc=auth(req.headers['x-session-token']);if(!acc)return json(res,401,{ok:false,error:'Login required'});const t=ensureWeekendTournament();if(t.status!=='live'&&t.status!=='finished')return json(res,200,{ok:true,assigned:false,status:t.status});const team=t.teams.find(tm=>tm.players.some(p=>p.playerId===acc.playerId));if(!team)return json(res,200,{ok:true,assigned:false,status:t.status});return json(res,200,{ok:true,assigned:true,team:{id:team.id,name:team.name,players:team.players.map(p=>p.name),captainId:team.captainId,designatedBatsman:team.designatedBatsman,isCaptain:team.captainId===acc.playerId,isDesignatedBatsman:team.designatedBatsman===acc.playerId}});
 }
 if(req.method==='POST'&&u==='/api/weekend/designate-batsman'){
   let body='';req.on('data',c=>body+=c);req.on('end',()=>{try{const acc=auth(req.headers['x-session-token']);if(!acc)return json(res,401,{ok:false,error:'Login required'});const m=JSON.parse(body||'{}'),t=ensureWeekendTournament(),team=t.teams.find(tm=>tm.captainId===acc.playerId);if(!team)return json(res,403,{ok:false,error:'Only captain can designate'});const target=team.players.find(p=>p.playerId===String(m.playerId));if(!target)return json(res,404,{ok:false,error:'Player not in your team'});team.designatedBatsman=target.playerId;saveWeekend();return json(res,200,{ok:true,message:'Designated: '+target.name+' (will bat 3 times)'})}catch(e){return json(res,400,{ok:false,error:e.message})}});return;
 }
...
async function startServer(){
  try{
    const storage=await initAccountPersistence();
    server.listen(PORT,()=>console.log(`Hand Cricket Arena multiplayer server running on port ${PORT} | account storage: ${storage}`));
  }catch(e){
    console.error('FATAL: PostgreSQL account storage could not be initialized:',e.message);
    process.exit(1);
  }
}
startServer();
